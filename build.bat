...
:BUILD_START
echo.
echo Building Toledo Atomchess Reloaded...
echo.
...
if exist toledo_atomchess_univac.exe del /Q toledo_atomchess_univac.exe
if exist toledo_atomchess_univac.o del /Q toledo_atomchess_univac.o
if exist *.o del /Q *.o
echo.
...
echo Compiling toledo_atomchess.c...
gcc -c -DUNIVAC -O2 -Wall toledo_atomchess.c -o toledo_atomchess_univac.o

if %ERRORLEVEL% NEQ 0 (
    echo ERROR: Failed to compile toledo_atomchess.c
    pause
    exit /b 1
)

echo Linking...
gcc -o toledo_atomchess_univac.exe toledo_atomchess_univac.o
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: toledo_atomchess_univac.exe

    REM Display file size
    for %%A in (toledo_atomchess_univac.exe) do (
    ...
    echo   - No Windows dependencies ^(windows.h, conio.h^)
...
REM Clean previous build artifacts
echo Cleaning previous build artifacts...
if exist toledo_atomchess_mingw.exe del /Q toledo_atomchess_mingw.exe
if exist toledo_atomchess.obj del /Q toledo_atomchess.obj
if exist *.o del /Q *.o
echo.
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: toledo_atomchess_mingw.exe

    REM Display file size
    for %%A in (toledo_atomchess_mingw.exe) do (
        echo File size: %%~zA bytes
    )
    echo.
    echo Optimization level: MAXIMUM (-O3 + LTO + native CPU^)
    echo.
    echo To play chess, type: toledo_atomchess_mingw.exe
    echo.
) else (
    echo.
    echo ========================================
...
cl /W4 /O2 /Fe:toledo_atomchess.exe toledo_atomchess.c
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: toledo_atomchess.exe
    echo.
    echo To play: toledo_atomchess.exe
) else (
    echo.
    echo ========================================
    echo   BUILD FAILED
